...
from llama_index.schema import NodeWithScore, NodeWithScore2
...
class LLMRerank(BaseNodePostprocessor):
    ...
    def postprocess_nodes(
        ...
        initial_results: List[NodeWithScore] = []
        for idx in range(0, len(nodes), self._choice_batch_size):
            print(f'BEFORE RERANK:{nodes}')
            nodes_batch = [
                node.node for node in nodes[idx : idx + self._choice_batch_size]
            ]
            scores_batch = [
                node.score for node in nodes[idx : idx + self._choice_batch_size]
            ]
            ...
            raw_choices, relevances, flag = self._parse_choice_select_answer_fn(
                raw_response, len(nodes_batch)
            )
            if flag:
                print(f'AFTER RERANK FAILURE: {nodes[ : self._top_n]}')
                ...
                choice_idxs = [int(choice) - 1 for choice in raw_choices]
                choice_nodes = [nodes_batch[idx] for idx in choice_idxs]
                choice_scores = [scores_batch[idx] for idx in choice_idxs]
                relevances =  relevances or [1.0 for _ in choice_nodes]
                initial_results.extend(
                    [
                        NodeWithScore2(node=node, score=[score, relevance])
                        for node, score, relevance in zip(choice_nodes, choice_scores, relevances)
                    ]
                )

        results = sorted(initial_results, key=lambda x: x.score[0] or 0.0, reverse=True)[
            : self._top_n
        ]
        print(f'RAW CHOICES: {raw_choices}')
        print(f'AFTER RERANK: {results}')
        return results
...
def default_parse_choice_select_answer_fn(
    answer: str, num_choices: int, raise_error: bool = True
) -> Tuple[List[int], Optional[List[float]]]:
    """Default parse choice select answer function."""
    answer_lines = answer.split("\n")
    answer_nums = []
    ...
    for answer_line in answer_lines:
        line_tokens = answer_line.split(",")
        if len(line_tokens) != 2 or len(line_tokens[0].split(":")) !=2 or len(line_tokens[1].split(":")) != 2:
            if not raise_error:
                continue
            else:
        ...
        if flag == True:
            ...
            answer_relevances = []
            print(answer)
            return answer_nums, answer_relevances, flag
        else:
            answer_num = int(line_tokens[0].split(":")[1].strip())
            if answer_num > num_choices:
                continue
